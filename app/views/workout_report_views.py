...
def get_workout_report(current_user, workout_plan_id):
    ...
    return {
        ...
        "sessions": [
            {
                "id": session.id,
                "scheduled_at": session.date.isoformat()
                if session.scheduled_at
                else None,
                "started_at": session.date.isoformat() if session.started_at else None,
                "completed_at": session.date.isoformat()
                ...
                "exercises": [
                    ...
                    for ws_ex in session.exercises
                ]
